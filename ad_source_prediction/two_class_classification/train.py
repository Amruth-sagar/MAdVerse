...
config = load_config('/home2/rishabh.s/AD_source_Prediction/classification/config.yaml')
...
def trainer(config, run_name=config['wandb']['run_name'], no_sweep = True):
    ...
    train_dataset = AdSource(images_dirs=[config['data']['train_dir']],
    ...
    valid_dataset = AdSource(images_dirs=[config['data']['val_dir']],
                            img_preprocessor=img_preprocessor,
                            of_num_imgs=None,
                            overfit_test=False,
                            augment_data=False)
